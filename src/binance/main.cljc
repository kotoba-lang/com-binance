(ns binance.main
  "Kotodama WASM entrypoint for the Binance clean-room actor (L4) — Clojure port.
  ...
  facts (`binance.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "binance")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"    :plural "orders"    :id-prefix "binance_ord"
    :fields [:symbol :orderId :clientOrderId :price :origQty :executedQty :cummulativeQuoteQty :status :type :side :timeInForce :transactTime]
    :required [:symbol :price]
    :coerce {:orderId :int :transactTime :int}
    :refs {}}
   {:entity "Trade"    :plural "trades"    :id-prefix "binance_tra"
    :fields [:symbol :orderId :price :qty :quoteQty :commission :commissionAsset :time :isBuyer :isMaker]
    :required [:symbol]
    :coerce {:orderId :int :time :int :isBuyer :bool :isMaker :bool}
    :refs {:orderId "Order"}}
   {:entity "Balance"  :plural "balances"  :id-prefix "binance_bal"
    :fields [:asset :free :locked]
    :required [:asset :free]
    :coerce {}
    :refs {}}
   {:entity "Account"  :plural "accounts"  :id-prefix "binance_acc"
    :fields [:makerCommission :takerCommission :canTrade :canWithdraw :canDeposit :updateTime]
    :required [:makerCommission :takerCommission]
    :coerce {:makerCommission :int :takerCommission :int :canTrade :bool :canWithdraw :bool :canDeposit :bool :updateTime :int}
    :refs {}}
   {:entity "Ticker"   :plural "tickers"   :id-prefix "binance_tic"
    :fields [:symbol :lastPrice :priceChange :priceChangePercent :highPrice :lowPrice :volume :quoteVolume :openTime :closeTime]
    :required [:symbol :lastPrice]
    :coerce {:openTime :int :closeTime :int}
    :refs {}}
   {:entity "Kline"    :plural "klines"    :id-prefix "binance_kli"
    :fields [:openTime :open :high :low :close :volume :closeTime :trades]
    :required [:openTime :open]
    :coerce {:openTime :int :closeTime :int :trades :int}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"binance.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "binance-compat" :tier tier :entities entities} 200])
...
(defn start! [] :binance-compat/ready)
